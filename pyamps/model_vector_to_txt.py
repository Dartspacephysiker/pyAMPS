...
except ImportError: # allow execution from inside the pyamps directory
    from sh_utils import SHkeys
    from coefficients import MODEL_VECTOR_LATEST, MODEL_COEFF_LATEST
...
def make_keys(NT = 65, MT = 3, NV = 45, MV = 3):
...
def model_vector_to_coefficients(model_vector, NT = 65, MT = 3, NV = 45, MV = 3):
    keys = make_keys(NT = NT, MT = MT, NV = NV, MV = MV)
    ...
    dataframes = [vector_to_df(m, keys) for m in np.split(model_vector, NTERMS)]
...
def main(model_vector_fn = MODEL_VECTOR_LATEST, coeff_fn = MODEL_COEFF_LATEST):
    model_vector = np.load(model_vector_fn)
    coefficients = model_vector_to_coefficients(model_vector)

    # write txt file
    with open(coeff_fn, 'w') as file:
        # header:
